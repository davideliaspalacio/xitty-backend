...
- 2026-07-09: iniciado cierre extra F6 en rama `feat/f6-reservation-outbox`: reservas confirmadas encolan `reservation_created` en `business_notification_outbox` respetando `notify_reservation_click`.
...
| F6 Preferencias notificaciones | Mergeado en `main`, cierre de reservas en rama                                                  | Backend #29 / Frontend #24 / Backend `feat/f6-reservation-outbox`      | Definir canal/proveedor real; por ahora queda outbox interno/pending.                                                                                                      |
...
- 2026-07-09: F6 cierre extra iniciado: reservas confirmadas escriben `reservation_created` en outbox respetando `notify_reservation_click`.
...
- 2026-07-09: F7 configuracion admin mergeada en backend #126 y frontend #26, con endpoints admin `GET/PATCH /admin/ranking/config`, validacion de pesos y UI `/admin/ranking`.
...
- Backend F7 config admin focused: `npm test -- ranking.controller.spec.ts ranking.service.spec.ts --runInBand` -> 2 suites / 25 tests OK.
- Backend full suite tras F7 config admin: `npm test -- --runInBand` -> 39 suites / 490 tests OK.
- Backend lint/build tras F7 config admin: `npx eslint "src/**/*.ts"` -> OK; `npm run build` -> OK.
- Frontend F7 config admin focused: `npm run test:run -- src/app/__tests__/admin-ranking.test.tsx src/features/admin/__tests__/api.test.ts` -> 2 files / 11 tests OK.
- Frontend full suite tras F7 config admin: `npm run test:run` -> 47 files / 224 tests OK.
- Frontend typecheck/lint/build tras F7 config admin: `npm run typecheck`, `npm run lint`, `npm run build` -> OK.
- Backend F6 reservation outbox enfocado: `npm test -- --runInBand src/modules/experiences/reservations.service.spec.ts` -> 1 suite / 15 tests OK.
...
- Backend category response dto lint: `npx eslint src/modules/places/dto/category-response.dto.ts` -> OK.
...
- Backend OG response dto lint: `npx eslint src/modules/places/dto/og-response.dto.ts` -> OK.
- Backend places smoke tests tras OG response dto: `npm test -- --runInBand src/modules/places/places.service.spec.ts` -> 1 suite / 19 tests OK.
- Backend build tras OG response dto lint: `npm run build` -> OK.
- Backend full lint tras OG response dto: 2 archivos / 2 errores / 0 warnings; solo queda deuda Prettier.
- Backend promotion response dto lint: `npx eslint src/modules/promotions/dto/promotion-response.dto.ts` -> OK.
