...
- 2026-07-09: PR tecnico pendiente: `chore/backend-lint-favorite-response-dto`; favorite response dto queda con lint dirigido limpio.
...
- Backend mock enrichment provider lint: `npx eslint src/modules/scraping/enrichment/providers/mock-enrichment-provider.ts` -> OK.
- Backend enrichment smoke tests: `npm test -- --runInBand src/modules/scraping/enrichment/enrichment.service.spec.ts` -> 1 suite / 14 tests OK.
- Backend build tras mock enrichment provider lint: `npm run build` -> OK.
- Backend full lint tras mock enrichment provider: 17 archivos / 22 errores / 0 warnings; baja deuda historica, no queda aun en verde global.
- Backend OpenAI enrichment provider lint: `npx eslint src/modules/scraping/enrichment/providers/openai-enrichment-provider.ts` -> OK.
...
- Backend discover controller lint: `npx eslint src/modules/scraping/public/discover.controller.ts` -> OK.
- Backend discover smoke tests: `npm test -- --runInBand src/modules/scraping/public/discover.service.spec.ts` -> 1 suite / 12 tests OK.
- Backend build tras discover controller lint: `npm run build` -> OK.
- Backend full lint tras discover controller: 14 archivos / 16 errores / 0 warnings; baja deuda historica, no queda aun en verde global.
- Backend curated item dto lint: `npx eslint src/modules/scraping/public/dto/curated-item.dto.ts` -> OK.
- Backend discover smoke tests tras curated item dto: `npm test -- --runInBand src/modules/scraping/public/discover.service.spec.ts` -> 1 suite / 12 tests OK.
- Backend build tras curated item dto lint: `npm run build` -> OK.
- Backend full lint tras curated item dto: 13 archivos / 14 errores / 0 warnings; baja deuda historica, no queda aun en verde global.
- Backend scheduler module lint: `npx eslint src/modules/scraping/scheduler/scheduler.module.ts` -> OK.
...
- Backend create conversation dto lint: `npx eslint src/modules/chat/dto/create-conversation.dto.ts` -> OK.
...
- Backend experience review response dto lint: `npx eslint src/modules/experiences/dto/experience-review-response.dto.ts` -> OK.
- Backend experience reviews smoke tests tras response dto: `npm test -- --runInBand src/modules/experiences/experience-reviews.service.spec.ts` -> 1 suite / 8 tests OK.
- Backend build tras experience review response dto lint: `npm run build` -> OK.
- Backend full lint tras experience review response dto: 7 archivos / 7 errores / 0 warnings; solo queda deuda Prettier.
- Backend experiences module lint: `npx eslint src/modules/experiences/experiences.module.ts` -> OK.
...
- Backend chat rate limit lint: `npx eslint src/modules/chat/rate-limit.service.spec.ts` -> OK.
- Backend chat rate limit tests: `npm test -- --runInBand src/modules/chat/rate-limit.service.spec.ts` -> 1 suite / 5 tests OK.
- Backend build tras chat rate limit lint: `npm run build` -> OK.
...
- Backend notification settings controller lint: `npx eslint src/modules/notification-settings/notification-settings.controller.ts` -> OK.
- Backend notification settings smoke tests: `npm test -- --runInBand src/modules/notification-settings/notification-settings.service.spec.ts` -> 1 suite / 3 tests OK.
- Backend build tras notification settings controller lint: `npm run build` -> OK.
...
- Backend reviews controller lint: `npx eslint src/modules/reviews/reviews.controller.ts` -> OK.
- Backend reviews smoke tests: `npm test -- --runInBand src/modules/reviews/reviews.service.spec.ts` -> 1 suite / 8 tests OK.
- Backend build tras reviews controller lint: `npm run build` -> OK.
...
- Backend location controller lint: `npx eslint src/modules/location/location.controller.ts` -> OK.
- Backend location smoke tests: `npm test -- --runInBand src/modules/location/location.service.spec.ts` -> 1 suite / 8 tests OK.
- Backend build tras location controller lint: `npm run build` -> OK.
- Backend scraping sources repo lint: `npx eslint src/modules/scraping/storage/scraping-sources.repo.ts` -> OK.
- Backend scraping sources repo tests: `npm test -- --runInBand src/modules/scraping/storage/scraping-sources.repo.spec.ts` -> 1 suite / 11 tests OK.
- Backend build tras scraping sources repo lint: `npm run build` -> OK.
- Backend experience list query dto lint: `npx eslint src/modules/experiences/dto/experience-list-query.dto.ts` -> OK.
- Backend experience list query smoke tests: `npm test -- --runInBand src/modules/experiences/experiences.service.spec.ts` -> 1 suite / 12 tests OK.
- Backend build tras experience list query dto lint: `npm run build` -> OK.
- Backend Supabase config lint: `npx eslint src/config/supabase.config.ts` -> OK.
- Backend build tras Supabase config lint: `npm run build` -> OK.
- Backend create experience dto lint: `npx eslint src/modules/experiences/dto/create-experience.dto.ts` -> OK.
- Backend create experience dto smoke tests: `npm test -- --runInBand src/modules/experiences/experiences.service.spec.ts` -> 1 suite / 12 tests OK.
- Backend build tras create experience dto lint: `npm run build` -> OK.
...
- Backend preferences service lint: `npx eslint src/modules/preferences/preferences.service.ts` -> OK.
- Backend preferences service tests: no existe spec de `PreferencesService`/`PreferencesController` en `src/**/*.spec.ts`; verificado con build.
- Backend build tras preferences service lint: `npm run build` -> OK.
- Backend source factory lint: `npx eslint src/modules/scraping/sources/source.factory.ts` -> OK.
- Backend source factory tests: `npm test -- --runInBand src/modules/scraping/sources/source.factory.spec.ts` -> 1 suite / 8 tests OK.
- Backend build tras source factory lint: `npm run build` -> OK.
...
- Backend enrichment service lint: `npx eslint src/modules/scraping/enrichment/enrichment.service.ts src/modules/scraping/enrichment/enrichment.service.spec.ts` -> OK.
- Backend enrichment service tests: `npm test -- --runInBand src/modules/scraping/enrichment/enrichment.service.spec.ts` -> 1 suite / 14 tests OK.
- Backend build tras enrichment service lint: `npm run build` -> OK.
...
- Backend scraping module spec lint: `npx eslint src/modules/scraping/scraping.module.spec.ts` -> OK.
- Backend scraping module spec tests: `npm test -- --runInBand src/modules/scraping/scraping.module.spec.ts` -> 1 suite / 4 tests OK.
- Backend build tras scraping module spec lint: `npm run build` -> OK.
- Backend localize lint: `npx eslint src/common/i18n/localize.ts` -> OK.
- Backend localize tests: `npm test -- --runInBand src/common/i18n/__tests__/localize.spec.ts` -> 1 suite / 16 tests OK.
- Backend build tras localize lint: `npm run build` -> OK.
- Backend chat response dto lint: `npx eslint src/modules/chat/dto/message-response.dto.ts` -> OK.
- Backend chat response dto smoke tests: `npm test -- --runInBand src/modules/chat/chat.service.spec.ts` -> 1 suite / 18 tests OK.
- Backend build tras chat response dto lint: `npm run build` -> OK.
- Backend reservation response dto lint: `npx eslint src/modules/experiences/dto/reservation-response.dto.ts` -> OK.
- Backend reservation response dto smoke tests: `npm test -- --runInBand src/modules/experiences/reservations.service.spec.ts` -> 1 suite / 13 tests OK.
- Backend build tras reservation response dto lint: `npm run build` -> OK.
...
- Backend recommendation item dto lint: `npx eslint src/modules/recommendations/dto/recommendation-item.dto.ts` -> OK.
- Backend recommendation item dto smoke tests: `npm test -- --runInBand src/modules/recommendations/recommendations.service.spec.ts` -> 1 suite / 11 tests OK.
- Backend build tras recommendation item dto lint: `npm run build` -> OK.
- Backend recommendations controller lint: `npx eslint src/modules/recommendations/recommendations.controller.ts` -> OK.
- Backend recommendations controller smoke tests: `npm test -- --runInBand src/modules/recommendations/recommendations.service.spec.ts` -> 1 suite / 11 tests OK.
- Backend build tras recommendations controller lint: `npm run build` -> OK.
- Backend quality scorer spec lint: `npx eslint src/modules/scraping/enrichment/quality-scorer.spec.ts` -> OK.
- Backend quality scorer spec tests: `npm test -- --runInBand src/modules/scraping/enrichment/quality-scorer.spec.ts` -> 1 suite / 12 tests OK.
- Backend build tras quality scorer spec lint: `npm run build` -> OK.
...
- Backend create slot dto lint: `npx eslint src/modules/experiences/dto/create-slot.dto.ts` -> OK.
- Backend create slot dto smoke tests: `npm test -- --runInBand src/modules/experiences/experiences.service.spec.ts` -> 1 suite / 12 tests OK.
- Backend build tras create slot dto lint: `npm run build` -> OK.
- Backend full lint: `npx eslint "src/**/*.ts"` -> falla por deuda historica restante documentada en `specs/LINT_DEBT.md`; baja a 19 archivos / 26 errores / 0 warnings.
