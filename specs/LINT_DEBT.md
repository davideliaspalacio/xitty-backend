...
- Archivos con problemas: 104.
- Errores: 1868.
- Warnings: 165.
- Errores auto-fixables: 461.
...
- Reduccion neta acumulada: -14 archivos con problemas, -842 errores y -82 warnings.

## Reglas principales

| Regla | Errores | Warnings | Lectura |
| --- | ---: | ---: | --- |
| `@typescript-eslint/no-unsafe-member-access` | 692 | 0 | Acceso a datos `any`, sobre todo mocks Supabase/tests. |
| `prettier/prettier` | 460 | 0 | Formato historico fuera de Prettier. |
| `@typescript-eslint/no-unsafe-call` | 286 | 0 | Llamadas sobre valores `any`. |
| `@typescript-eslint/no-unsafe-assignment` | 279 | 0 | Asignaciones desde `any`. |
| `@typescript-eslint/no-unsafe-argument` | 0 | 164 | Argumentos `any` en tests/servicios. |
| `@typescript-eslint/no-unsafe-return` | 125 | 0 | Retornos `any` sin tipar. |
| `@typescript-eslint/require-await` | 10 | 0 | Funciones async sin await. |
| `@typescript-eslint/no-unused-vars` | 6 | 0 | Variables/imports historicos sin usar. |
