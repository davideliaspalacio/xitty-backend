...
export class LocalPicksService {
  ...
  async create(userId: string, userRole: string, dto: CreateLocalPickDto) {
  ...
  async update(id: string, userRole: string, dto: UpdateLocalPickDto) {
    this.assertAdmin(userRole);
    ...
    const fields = [
      'curator_name', 'pick_tag', 'short_pitch', 'hero_image_url',
      'week_starts_at', 'week_ends_at', 'position', 'is_active',
    ] as const;
