...
function createChain(result: any) {
  const chain: any = {};
  const methods = [
    'from', 'select', 'insert', 'eq', 'maybeSingle', 'single',
  ];
...
describe('MetricsService', () => {
  ...
  describe('track', () => {
    it('registra una interaccion de un usuario autenticado', async () => {
      ...
      await expect(
        service.track('place-1', 'user-1', { interaction_type: InteractionType.CALL_CLICK }),
      ).resolves.toBeUndefined();
    });

    it('registra una interaccion anonima (user_id null)', async () => {
      supabase._on({ id: 'place-1' });
      supabase._on(null);

      await expect(
        service.track('place-1', null, { interaction_type: InteractionType.PROFILE_VIEW }),
      ).resolves.toBeUndefined();
    });
    ...
    it('lanza 404 si el place no existe', async () => {
      supabase._on(null);
      await expect(
        service.track('x', null, { interaction_type: InteractionType.PROFILE_VIEW }),
  ...
  describe('getSummary', () => {
    it('owner ve metricas con comparativa con periodo anterior', async () => {
      ...
      supabase._onRpc([
        {
          total_views: 100, total_calls: 20, total_whatsapp: 15,
          total_reservations: 5, total_directions: 30, total_promo_views: 10,
          total_interactions: 180, prev_total_interactions: 150,
          change_percent: 20.0,
        },
      ]);

      const result = await service.getSummary(
        'place-1', 'owner-1', 'business',
    ...
    it('rechaza a quien no es owner ni admin', async () => {
      ...
      await expect(
        service.getSummary('place-1', 'user-X', 'business', '2026-04-01', '2026-04-30'),
    ...
    it('admin puede ver metricas de cualquier place', async () => {
      supabase._onRpc([
        {
          total_views: 0, total_calls: 0, total_whatsapp: 0,
          total_reservations: 0, total_directions: 0, total_promo_views: 0,
          total_interactions: 0, prev_total_interactions: 0,
          change_percent: 0,
        },
      ]);

      const result = await service.getSummary(
        'place-1', 'admin-1', 'admin', '2026-04-01', '2026-04-30',
      );
  ...
  describe('getTimeseries', () => {
    it('devuelve buckets por dia con conteos casteados a numero', async () => {
      supabase._on({ owner_id: 'owner-1' });
      supabase._onRpc([
        { bucket: '2026-04-01T00:00:00Z', views: '10', calls: '2', whatsapp: '1', reservations: '0', directions: '3', promo_views: '0', total: '16' },
      ]);
